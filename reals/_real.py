...
class Real:
    ...
    @staticmethod
    def from_fractional(x: Union[int, Fraction, Decimal]):
        if isinstance(x, int):
            return Real.from_int(x)

        if not (isinstance(x, Fraction) or isinstance(x, Decimal)):
            raise TypeError(f'Expected int, Fraction, or Decimal, got {type(x)}')
